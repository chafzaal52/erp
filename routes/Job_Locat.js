...
router.get('/', function (req, res, next) {
    res.send("Job Work properly");
});


router.get('/GetList/:CompanyID', function (req, res, next) {
    let CompanyID=req.params.CompanyID
    let query="select * from tb_JOB_Loc where CompanyID="+CompanyID;
...
router.get('/GetById/:Id', function (req, res, next) {
    ...
    var query="select *  from tb_JOB_Loc where JobCode="+Id+";";
    new sql.connect(config).then(pool=> {   
        return pool.request()
                .query(query)
                .then(result => {
   ...
   router.post('/InsertUpdate', function (req, res, next) {
    ...
    sql.connect(config).then(response => {
        ...
        .execute('sp_InsertUpdateJob_Loc')
    }).then(result => {
        res.send("Success");
    }).catch(err => {
        res.send({ err });
...
router.delete('/Delete/:Id', function (req, res, next) {
    var Id= req.params.Id;
    var query="delete from tb_JOB_Loc where JobCode="+Id+";";
